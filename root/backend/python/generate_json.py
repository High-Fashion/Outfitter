...
def get_random_outfit(item_id):
    if (randint(0, 1) == 0):
        ...
        return {
            "user": randint(0, 10),
            "tops": [get_random_item("top", (10 * item_id) + j) for j in range(randint(1, 5))],
            "bottoms": get_random_item("bottom", (10 * item_id) + 1),
            "shoes": get_random_item("shoe", (10 * item_id) + 1),
            "accessories": [get_random_item("accessory", (10 * item_id) + j) for j in range(randint(0, 10))],
        }
    else:
        # one_piece outfit
        return {
            "user": randint(0, 10),
            "tops": [get_random_item("top", (10 * item_id) + j) for j in range(randint(0, 5))],
            "one_piece": get_random_item("one_piece", (10 * item_id) + 1),
            "shoes": get_random_item("shoe", (10 * item_id) + 1),
            "accessories": [get_random_item("accessory", (10 * item_id) + j) for j in range(randint(0, 10))],
        }
...
def main(args):
    ...
    if output_file == None:
        output_file = ((item_type + "s") if (item_type != "accessory") else "accessories") + ".json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate items in a json file")
    parser.add_argument("-i", "--item_type", required=True, choices=["shoe", "top", "bottom", "one_piece", "accessory", "outfit"], help="The type of items you would like to generate")
    parser.add_argument("-n", "--num_items", type=int, default=100, help="The number of items you would like to generate. Defaults to 100")
    parser.add_argument("-o", "--output_file", help="The path of the output json file. Defaults to '<item_type>s.json'")
