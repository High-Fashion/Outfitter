...
function changeColorCodesOutfit(outfit) {
  var newOutfit = {
    ...
    left: {
      upper_arm: outfit["left"]["upper_arm"].map(item => changeColorCodes(item)),
      forearm: outfit["left"]["forearm"].map(item => changeColorCodes(item)),
      wrist: outfit["left"]["wrist"].map(item => changeColorCodes(item)),
      hand: {
        thumb: outfit["left"]["hand"]["thumb"].map(item => changeColorCodes(item)),
        index: outfit["left"]["hand"]["index"].map(item => changeColorCodes(item)),
        middle: outfit["left"]["hand"]["middle"].map(item => changeColorCodes(item)),
        ring: outfit["left"]["hand"]["ring"].map(item => changeColorCodes(item)),
        pinky: outfit["left"]["hand"]["pinky"].map(item => changeColorCodes(item)),
      },
      thigh: outfit["left"]["thigh"].map(item => changeColorCodes(item)),
      ankle: outfit["left"]["ankle"].map(item => changeColorCodes(item)),
    },
    right: {
      upper_arm: outfit["right"]["upper_arm"].map(item => changeColorCodes(item)),
      forearm: outfit["right"]["forearm"].map(item => changeColorCodes(item)),
      wrist: outfit["right"]["wrist"].map(item => changeColorCodes(item)),
      hand: {
        thumb: outfit["right"]["hand"]["thumb"].map(item => changeColorCodes(item)),
        index: outfit["right"]["hand"]["index"].map(item => changeColorCodes(item)),
        middle: outfit["right"]["hand"]["middle"].map(item => changeColorCodes(item)),
        ring: outfit["right"]["hand"]["ring"].map(item => changeColorCodes(item)),
        pinky: outfit["right"]["hand"]["pinky"].map(item => changeColorCodes(item)),
      },
...
exports.generateSimilarOutfits = async (req, res) => {
  ...
  var desired = await Outfit.findById(id);
  if (!desired) {
  ...
  py = spawn("python3", ['python/recommend/recommend_outfit.py'], {shell: true})
  py.stdout.setEncoding('utf8');
  var jsonOut = ""
  var outfits = await new Promise(resolve => {
    py.stdout.on('data', function(data) {
      ...
      switch (data) {
        ...
        default:
          jsonOut = data
          var outfits = JSON.parse(jsonOut)
          resolve(outfits)
      }
    });
  ...
  var newOutfits = await Promise.all(outfits.map(outfit => {
...
exports.updateOne = (req, res) => {
  const id = req.params.id;
  ["hair", "head", "eyes", "nose", "ear", "mouth", "neck", "torso", "back",
    "left", "right", "legs", "waist", "hips", "feet"].map(slot => {
      if (!Object.keys(req.body).includes(slot)) {
        req.body[slot] = []
      }
    })
  console.log(req.body)
  Outfit.findByIdAndUpdate(id, { $set: { ...req.body } }).then(data => {
    if (!data) {
      return res.status(404).send({
        message: `Cannot update outfit with id=${id}. Maybe item was not found!`,
      });
    }
    return res.status(200).send({ message: "Outfit was updated successfully." });
  }).catch((err) => {
